...
//	Rigidbody enem;
	...
	public float audioDelay = 20f;
	NavMeshAgent agent;
	void Awake(){
		enemy = transform;
		//enem = GetComponent<Rigidbody>();
	}
	...
	void Update () {
		...
		if (!(state.IsName ("FallB") || state.IsName ("FallL") || state.IsName ("FallR") || state.IsName ("HitIdle") || state.IsName ("HitNear") || state.IsName ("Hit") || state.IsName ("Dead"))) {
			if (distance <= 1.0f && state.IsName ("Run")) {
				anim.Play ("Attack00");
				...
				//enem.velocity = new Vector3(velocity.x, enem.velocity.y, velocity.z);
